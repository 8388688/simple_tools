...
* 4.8-beta1
    * 2025-07-22
        * 标准化更新日志（其实也不是那么标准啦）
        * 移除 game_disposition.py, gui_extend.py
        * 重命名 encr_decr.py -> encrypt.py
        * 移除全部 I/O 操作
        * data_base
            * 移除 `usernameList(variable), EMPTY_UUID(variable)`
        * data_process
            * 移除 `list2str`
        * maths
            * 移除 `convert_system, dec2r_convert, decomposition, gcd_legacy, saving_decomposition(variable), generate_prime_range(variable)`
            * 弃用 `get_prime_range`
        * system_extend
            * 移除 `file_suffix`, `safe_md`
        * misc
            * 移除 `clear_module_cache`
            * 将 gui_extend.py 的 `check_and_install_package` 移入 misc.py 并稍做修改，移除 `noWarning` 参数
            * 移除 `get_update` 相关的代码
        * times
            * 弃用 `timestamp`
...
* 4.5-pre3 \[2024-02-15-11-10-33\] \(version_code: 20240215001\)
    * Fix: Create workspace in current directory when %APPDATA% is not defined(此功能于 4.7 Release 版本被移除).
...
* 4.4-beta2 \[2023-02-18-17-55-10\] \(version_code: 20230218001\)
    ...
    * Update `times.wait()`.
    * Move `passed.pass_()` to [default.py][misc].
...
* 4.4-pre1 \[2023-02-03-20-10-46\] \(version_code: 20230203002\)
    * Owned version code (yyyymmdd\<num\>)
      example: 20230203001
    * Auto upgrade itself(此功能在 4.8 版本弃用，4.9 版本被移除)
...
* 4.3-alpha1 \[2023-01-31 20:21:59\]
    ...
    * Fix a bug: Always repeat tip `<filepath> 文件不存在或已损坏\n正在重制此文件`.
...
* 4.2-beta3 \[2023-01-26 11:11:20\]
    ...
    * Delete 5 variables in [encr_decr.py][encrypt].
...
* 4.0-pre1 \[2022-10-3 18:20:15\]
    ...
    * upload these package to [GitHub](https://github.com/)
...
* 3.4.6 更新: \[20220925\]
    ...
    * 在 [system_extend.py][system_extend] 中修正打印无用的空行的 bug
* 3.4.5 更新: \[20220925\]
* 3.4.4 更新: \[20220918\]
    * 修正 [system_extend.py][system_extend] 中一些错误
* 3.4.3 更新: \[20220918\]
* 3.4.2 更新: \[20220918\]
    * 改名 [data_handle.py][data_process] 和 [maths.py][maths] 函数名称
* 3.4.1 更新: \[20220918\]
* 3.4 更新: [maths 数学函数更新][maths] \[20220918\]
    * 加入 getPrime, get_prime_range_of_generator 函数
    * rename getPrime to get_prime_range,

## 3.3.x
* 3.3.8 更新: \[from 20220901 to 20220912\]
* 3.3.7 更新: \[20220825\]
    * 在 [default.py][misc] 中加入 clear_module1_cache 函数
* 3.3.6 更新: \[20220825\]
* 3.3.5 更新: \[20220824\]
...
## 2.9.x
* 2.9.1 更改: \[20220814\]
    * 修复 ~~game_disposition.py~~ 中 Users 类的少量 bug
* 2.9 更改: game_disposition 更新 \[20220813\]
    * 大规模更新 ~~game_disposition.py~~ 中的 Users 类
        * 用户
            * 用户体验
                * 退出时将用户信息存储到文件中
                * 现在登陆时需要密码
...
* 2.8 更改: [程序结构更新] \[20220812\]
    * 新建 ~~game_disposition.py~~ 文件，并将 User 类 和 Person 类 移到 ~~game_disposition.py~~ 中
...
* 2.6 更改: [加密更新][hash] \[20220801\]
    * 更新 md5_encryption
    * 修正 normalEncryption 的 bug, 并加入 coding 参数 `['auto', 'f_byte', 'f_int']` 三个可选项

## 2.5.x
* 2.5.1 更改: \[20220727\]
    * 加入 md5_encryption
    * 更新 normal_encryption 函数, 修复一些 bug
    * 标准化 [更新日志][changelog] 格式
* 2.5 更改: [system_ 更新][system_extend] \[20220404\]
* 在 [system_.py][system_extend] 下创建 File 类
* 添加 system_pro 变量声明当前操作系统，（4.？版本被移除）
...
* 2.4 更改: [times 更新][times] \[20220305\]
    ...
    * 在 times.py 中创建 get_false_unix_time 函数
    * 移动 wait 函数到 times.py 中
...
* 2.2 更改: \[20220205\]
    ...
    * 删除 `default.py` 中的 createToplevel
...
* 2.0 更改: \[20220203更早, 下同\]
    * 创建 file_remove, 大规模更新 [system_.py][system_extend]
...
* 1.7 更改:
    ...
    * 删除 `__init__.py` 的 passed, EuclideanAlgorithm.
