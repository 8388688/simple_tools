...
__all__ = ['File',
           ...
           'get_file_path', 'get_file_path_of_generator',
...
def file_remove(file_path=fp, all_files=False, all_folders=False,
    ...
    for a000 in get_file_path_of_generator(new_path, abspath=True, folders=all_folders, all_files=all_files):
...
def get_file_path_of_generator(file_path=fp, abspath=NULL, folders=False, __deep=0, **kwargs):
    """获取文件路径
    ...
    for file in fx:
        if os.path.isfile(os.path.join(file_path, file)):
            ...
            for a000 in get_file_path_of_generator(
                    os.path.join(file_path, file) + '/',
                    abspath=abspath, folders=folders, from_size=from_size, to_size=to_size,
                    suffix=suffix, include=include, exclude=exclude, case_sensitive=case_sensitive):
                yield a000
            if folders:
...
def get_file_suffix(file_path=fp, sort=True, show_details=False):
    ...
    for i in get_file_path_of_generator(fp_, abspath=False):
...
def safe_md(file_name_or_file_path, quiet=False):
    ...
    try:
        ...
        lfe.write(tell)
        os.mkdir(file_name_or_file_path) if not os.path.exists(file_name_or_file_path) else (
            print(tell) if not quiet else pass_())
    except FileNotFoundError:
